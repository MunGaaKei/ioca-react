...
const VirtualCell = memo(function VirtualCell({ column, data, row, col, isHeader, cellEllipsis, sortBy, sortType, resizable, onWidthChange }: VirtualCellProps) {
    ...
    return (
        ...
        >
            ...
            {isHeader && resizable && <Resize index={col} onWidthChange={onWidthChange} />}
        </div>
    );
});
...
const VirtualRow = memo(function VirtualRow({
    ...
    const handleCellClick = useCallback(
        (e: any) => {
            ...
            const el = (e.target as HTMLElement | null)?.closest?.(".i-datagrid-cell[data-col]") as HTMLElement | null;
    ...
    return (
        ...
        >
            {columns.map((c, col) => (
                <VirtualCell key={c.id} column={c} data={rowData} row={rowNum} col={col} cellEllipsis={cellEllipsis} sortBy={sortBy} sortType={sortType} resizable={resizable} onWidthChange={onWidthChange} />
            ))}
        </div>
    );
});
...
export default function VirtualDatagrid(props: VirtualDatagridProps) {
    ...
    useEffect(() => {
        ...
        const update = () => {
            if (rafRef.current.viewport) cancelAnimationFrame(rafRef.current.viewport);
    ...
    const headerCells = useMemo(() => columns.map((c, col) => <VirtualCell key={c.id} column={c} row={0} col={col} isHeader cellEllipsis={cellEllipsis} sortBy={sortBy} sortType={sortType} resizable={resizable} onWidthChange={onWidthChange} />), [cellEllipsis, columns, onWidthChange, resizable, sortBy, sortType]);
    ...
    const loaderNode = useMemo<ReactNode>(() => virtual.loader ?? <Loading className="my-12" />, [virtual.loader]);
    ...
    return (
        ...
        >
            {header && (
                <div ref={headerRef} className="i-datagrid-virtual-header" onScroll={handleHeaderScroll} style={headerWrapStyle}>
                    <div ref={headerInnerRef} className="i-datagrid-header i-datagrid-row" style={headerInnerStyle} onClick={handleHeaderClick}>
                        {headerCells}
                    </div>
                </div>
            )}

            <VirtualList listRef={listRef} rowCount={rows.length + (virtual.hasMore ? 1 : 0)} rowHeight={virtual.rowHeight} overscanCount={Math.max(3, virtual.threshold ?? 8)} rowProps={rowProps as any} style={listStyle} onScroll={handleBodyScroll} onRowsRendered={handleRowsRendered} rowComponent={VirtualRow as any} />

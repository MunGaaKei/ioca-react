...
export const Options = (props: ISelectOptions) => {
    const { value: val, options, filter, filterPlaceholder, multiple, empty = <Empty />, onSelect, onFilter } = props;
        ...
        >
            {filter && multiple && (
                <div className="i-select-filter">
                    <Icon icon={<SearchRound />} className="color-8 ml-8 my-auto" size="1.2em" />
                    <input type="text" className="i-input" placeholder={filterPlaceholder} onChange={onFilter} />
                </div>
            )}
            ...
            {options.map((option, i) => {
                ...
                return (
                    <List.Item key={(value as string) || i} active={isActive} type="option" onClick={() => onSelect?.(value, option)} disabled={disabled}>
                        {multiple && <Icon icon={<CheckRound />} className="i-select-option-check" size="1em" />}
                        {label}
                    </List.Item>
                );
            })}
...
export const activeOptions = (options: TOption[] = [], value: any[] = [], max = 3) => {

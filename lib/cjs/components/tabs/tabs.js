...
const Tabs = ((props) => {
    ...
    react.useEffect(() => {
        const sanitizeContent = (node) => {
            ...
            if (!("$$typeof" in node))
                return node;
            return radash.pick(node, ["props", "type", "$$typeof", "ref"]);
        };
        if (!items) {
            if (!children) {
                state.tabs = [];
    ...
    return (jsxRuntime.jsxs("div", { className: classNames__default("i-tabs", { flex: vertical, [`i-tabs-${type}`]: type !== "default" }, className), ...rest, children: [jsxRuntime.jsxs("div", { className: classNames__default("i-tab-navs-container", {
                ...
                }), children: [prepend, jsxRuntime.jsxs("div", { ref: navsRef, className: classNames__default("i-tab-navs", `justify-${navsJustify}`), children: [state.tabs.map((tab, i) => {

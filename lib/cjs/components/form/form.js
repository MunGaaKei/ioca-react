...
const Form = (props) => {
    ...
    react.useEffect(() => {
        PubSub__default.default.subscribe(`${form.id}:change`, (evt, v) => {
            onChange?.(v.name, v.value);
        });
        return () => {
            PubSub__default.default.unsubscribe(`${form.id}:change`);
        };
    }, []);
    return (jsxRuntime.jsx(context.default, { value: form, children: jsxRuntime.jsx("form", { style: {
            ...
            }, className: classNames__default.default("i-form", className, {

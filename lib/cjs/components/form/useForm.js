...
class IFormInstance {
    ...
    set(field, value) {
        ...
        if (typeof field === "string") {
            ...
            PubSub__default.default.publish(`${id}:set:${field}`, value);
            return;
        }
        Object.keys(field).map((name) => {
            this.data[name] = field[name];
            this.cacheData[name] = field[name];
            PubSub__default.default.publish(`${id}:set:${name}`, field[name]);
        });
    }
    delete(field) {
    ...
    clear() {
        ...
        Object.keys(this.data).map((name) => {
            PubSub__default.default.publish(`${this.id}:set:${name}`, undefined);
    ...
    async validate(field) {
        ...
        if (field) {
            ...
            if (!isValid) {
                PubSub__default.default.publish(`${id}:invalid:${field}`, {
                    message: rule.message,
                    status: "error",
                });
                return false;
            }
            PubSub__default.default.publish(`${id}:invalid:${name}`, {
                message: null,
                status: "normal",
            });
            return true;

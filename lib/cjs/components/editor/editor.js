...
const Editor = (props) => {
    ...
    react.useImperativeHandle(ref, () => {
        return {
            setValue(html) {
                if (!editorRef.current)
                    return;
                const safeHtml = xss__default.default(html, controls.xssOptions);
                editorRef.current.innerHTML = safeHtml;
            },
            getSafeValue() {
                const html = editorRef.current?.innerHTML ?? "";
                return xss__default.default(html, controls.xssOptions);
            },
        };
    });
    const handleInput = (e) => {
    ...
    return (jsxRuntime.jsxs("div", { className: classNames__default.default("i-editor", className, {
